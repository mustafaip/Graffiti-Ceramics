...
class ShopifyOperationsWizard(models.TransientModel):
    ...
    def _execute_import_products(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "custom_odoo_shopify_connector_basic.action_shopify_product_queue"
        )
    ...
    def _execute_import_customers(self):
        ...
        try:
            action = self.env["ir.actions.act_window"]._for_xml_id(
                "custom_odoo_shopify_connector_basic.action_shopify_customer_import_queue"
            )
        except ValueError:
            action = {
    ...
    def _execute_import_orders(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "custom_odoo_shopify_connector_basic.action_shopify_order_queue"
        )
        action["context"] = dict(
            self.env.context,
    ...
    def _execute_import_shipped_orders(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "custom_odoo_shopify_connector_basic.action_shopify_import_queue"
        )
        action["res_id"] = queue.id
        action["view_mode"] = "form"
        return action
    ...
    def _execute_import_inventory(self):
        ...
        try:
            action = self.env["ir.actions.act_window"]._for_xml_id(
                "custom_odoo_shopify_connector_basic.action_shopify_import_stock_wizard"
            )
        except ValueError:
