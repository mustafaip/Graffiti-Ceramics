...
def license_is_active(env):
    """
    Full license check (preferred over calling the mixin only — harder to monkey-patch in one place).
    Missing/invalid => trial mode (False). Never raises.
    If license_expiry + key change (renewal), clear my_module.license_fingerprint once so the new pair can bind.
    """
    icp = env["ir.config_parameter"].sudo()
    raw_expiry = (icp.get_param("my_module.license_expiry") or "").strip()
    if not raw_expiry:
        _logger.info("Module running in test mode (limited features).")
        return False
    try:
        expiry = fields.Date.from_string(raw_expiry)
    except Exception:
        _logger.info("Module running in test mode (limited features).")
        return False
    try:
        if expiry < fields.Date.today():
            _logger.info("Module running in test mode (limited features).")
            return False
    except Exception:
        _logger.info("Module running in test mode (limited features).")
    ...
    db_uuid = (icp.get_param("database.uuid") or "").strip()
    key = (icp.get_param("my_module.license_key") or "").strip()
    if not db_uuid or not key:
        _logger.info("Module running in test mode (limited features).")
        return False

    fp = compute_license_fingerprint(db_uuid, expiry)
    stored_fp = (icp.get_param(_LICENSE_FINGERPRINT_PARAM) or "").strip()
    if stored_fp and fp != stored_fp:
        _logger.info("Module running in test mode (limited features).")
        return False

    expected = compute_license_key_hex(db_uuid, expiry)
    if not expected or not hmac.compare_digest(expected, key):
        _logger.info("Module running in test mode (limited features).")
        return False

    if not stored_fp:
        icp.set_param(_LICENSE_FINGERPRINT_PARAM, fp)
...
def license_is_active_strict(env):
    """Require database UUID present, then full license check (extra friction vs mixin-only bypass)."""
    icp = env["ir.config_parameter"].sudo()
    db_uuid = (icp.get_param("database.uuid") or "").strip()
    if not db_uuid:
        return False
    return license_is_active(env)


def trial_batch_limit():
    """Slightly variable trial batch size (optional degradation escalation)."""
    return 2 if fields.Date.today().day % 2 == 0 else 1
