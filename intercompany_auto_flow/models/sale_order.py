...
class SaleOrder(models.Model):
    ...
    # ── Smart button: IC invoice on the SUPPLYING company's SO ───────────────
    # The customer invoice lives in the supplying company. The supplying SO
    # is linked via intercompany.flow.log.intercompany_sale_id.
    # We look up the log record where this SO is the intercompany_sale_id
    # and surface its customer_invoice_id.
    ...
    @api.depends('is_intercompany_so')
    def _compute_intercompany_invoice_count(self):
        FlowLog = self.env['intercompany.flow.log'].sudo()
        for order in self:
            if order.is_intercompany_so:
                # This SO is in the supplying company — find its flow log
                log = FlowLog.search([('intercompany_sale_id', '=', order.id)], limit=1)
                order.intercompany_invoice_count = 1 if log and log.customer_invoice_id else 0
            else:
                order.intercompany_invoice_count = 0

    def action_view_intercompany_invoices(self):
        """Open the customer invoice for this intercompany SO (supplying company view)."""
        self.ensure_one()
        FlowLog = self.env['intercompany.flow.log'].sudo()
        log = FlowLog.search([('intercompany_sale_id', '=', self.id)], limit=1)
        if not log or not log.customer_invoice_id:
            return
        return {
            'name': _('Intercompany Invoice'),
            'type': 'ir.actions.act_window',
            'res_model': 'account.move',
            'view_mode': 'form',
            'res_id': log.customer_invoice_id.id,
        }
