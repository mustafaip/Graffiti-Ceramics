...
class AccountInvoice(models.Model):
    ...
    def open_pdc_payment(self):
        action = self.env.ref('jm_pdc.sh_pdc_payment_menu_action').read()[0]
    ...
    def _compute_total_pdc(self):
        for rec in self:
            ...
            for pdc in rec.pdc_payment_ids:
                if pdc.state == 'done':
                    rec.total_pdc_received += pdc.payment_amount
                elif pdc.state == 'cancel':
                    rec.total_pdc_cancel += pdc.payment_amount
                else:
                    rec.total_pdc_pending += pdc.payment_amount
            rec.total_pdc_payment = (
                rec.total_pdc_pending + rec.total_pdc_received + rec.total_pdc_cancel
            )
    ...
    def _compute_pdc_payment_invoice(self):
        for move in self:
            pdcs = self.env['pdc.wizard'].search([
                '|',
                ('invoice_id', '=', move.id),
                ('invoice_ids.id', '=', move.id),
            ])
            move.pdc_payment_ids = [(6, 0, pdcs.ids)] if pdcs else [(5, 0, 0)]
